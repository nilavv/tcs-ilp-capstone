...
@Component({
  selector: 'app-kids-items',
  templateUrl: './kids-items.component.html',
  styleUrls: ['./kids-items.component.css']
})
export class KidsItemsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.talkWithDbService.getAllItems().subscribe((data)=>{
      ...
      {
        if(this.temp[i]['itemCategory']=="kids")
        {
    ...
    this.talkWithDbService.countAllOrders().subscribe((data)=>{
      ...
      {
        this.currentCartCount+=this.currentOrder[i]['item']['itemQuantity'];
